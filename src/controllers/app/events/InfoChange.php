...
class InfoChange {
    public static function on_email_change($errors, $user_new_details) {
        ...
        if (!$user) {
            wc_clear_notices();
            Router::redirect('/verification/?action=error&msg=' . urlencode(__('We could not find your account data. Check your details and try again.', 'verifywoo')));
        }
        ...
        if (!$mail)  Router::redirect('/verification/?action=success&msg=' . urlencode(__('There was an issue sending your verification token. Please try again.', 'verifywoo')));
